...
class Macys_Scraper:
    def __init__(self):
        self.urls = [
            "https://www.macys.com/shop/handbags-accessories/all-designer-brands/Brand/kate%20spade%20new%20york%7CMichael%20Kors?id=69603",
        ]
    ...
    def refine_links_collection(self):
        ...
        link_col = db['handbags_product_links']
        item_col = db['handbags']
    ...
    def save_product_links(self):
        ...
        for item in self.driver.find_elements_by_xpath("//ul[@class='items large-block-grid-3']/li/div/div/a"):
            if db.handbags_product_links.find({"url": item.get_attribute('href')}).count() ==  0:
                print(item.get_attribute('href'))
                db.handbags_product_links.insert({"url": item.get_attribute('href')})
        mongo.close()
