...
class Macys_Scraper:
    def __init__(self):
        self.urls = [
            "https://www.macys.com/shop/womens-clothing/womens-tops/Brand,Special_offers/Calvin%20Klein%7CLauren%20Ralph%20Lauren%7CMichael%20Kors%7CTommy%20Hilfiger,Clearance%2FCloseout%7CLast%20Act?id=255"
        ]
    ...
    def save_product_links(self):
        ...
        for item in self.driver.find_elements_by_xpath("//ul[@class='items large-block-grid-3']/li/div/div/a"):
            if db.product_links.find({"url": item.get_attribute('href')}).count() ==  0:
                ...
                db.product_links.insert({"url": item.get_attribute('href')})
        mongo.close()
