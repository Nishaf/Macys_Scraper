...
class Macys_Scraper:
    def __init__(self):
        self.urls = [
            "https://www.macys.com/shop/womens-clothing/dresses/Brand,Special_offers/Calvin%20Klein%7CLauren%20Ralph%20Lauren%7CMichael%20Kors,Clearance%2FCloseout%7CLast%20Act?id=5449",
        ]
    ...
    def refine_links_collection(self):
        ...
        link_col = db['dresses_product_links']
        item_col = db['dresses']
    ...
    def save_product_links(self):
        ...
        for item in self.driver.find_elements_by_xpath("//ul[@class='items large-block-grid-3']/li/div/div/a"):
            if db.dresses_product_links.find({"url": item.get_attribute('href')}).count() ==  0:
                print(item.get_attribute('href'))
                db.dresses_product_links.insert({"url": item.get_attribute('href')})
        mongo.close()
