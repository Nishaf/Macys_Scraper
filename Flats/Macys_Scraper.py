...
class Macys_Scraper:
    def __init__(self):
        self.urls = [
            "https://www.macys.com/shop/shoes/flats/Brand,Women_shoe_size_t/kate%20spade%20new%20york%7CMichael%20Kors%7CCole%20Haan,6%7C6.5%7C7%7C7.5%7C8%7C8.5%7C9?id=50295&EFCKEY=%7B%7D",
        ]
    ...
    def refine_links_collection(self):
        ...
        link_col = db['flats_product_links']
        item_col = db['flats']
    ...
    def save_product_links(self):
        ...
        for item in self.driver.find_elements_by_xpath("//ul[@class='items large-block-grid-3']/li/div/div/a"):
            if db.flats_product_links.find({"url": item.get_attribute('href')}).count() ==  0:
                print(item.get_attribute('href'))
                db.flats_product_links.insert({"url": item.get_attribute('href')})
        mongo.close()
