...
class ProductsScraper:
    ...
    def get_description(self, soup):
        try:
            return (soup.find('section', attrs={
                'class': 'product-details-content ui-accordion-content ui-helper-reset no-division'}).text).strip()
        except:
            return None
    ...
    def store_data(self):
        ...
        doc = {
            'size': size_list,
            'price': price_list,
            'tag': ['Product Type_Pants', 'Brand_' + brand, 'Color_' + color_name],
        ...
        print(self.title)
        print(doc)
        if self.title or description or images is None:
